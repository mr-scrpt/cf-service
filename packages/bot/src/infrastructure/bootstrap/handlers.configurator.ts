...
export class HandlersConfigurator {
  configureHandlers(flows: ApplicationFlows, wizardEngine: WizardEngine, container: DIContainer) {
    ...
    const handlers: Array<{ action: CallbackAction; handler: CallbackHandler<unknown> }> = [
      ...
      {
        action: CallbackAction.DOMAIN_MANAGEMENT,
        handler: new DomainMenuHandler(flows.domainMenu),
      },
      {
        action: CallbackAction.DNS_CREATE_SELECT_DOMAIN,
        handler: new DnsCreateSelectDomainHandler(flows.createDnsFlow),
      },
      {
        action: CallbackAction.DNS_CREATE_SELECT_TYPE,
        handler: new DnsCreateSelectTypeHandler(flows.createDnsFlow),
      },
      {
        action: CallbackAction.DNS_SELECT_TYPE,
        handler: new DnsSelectTypeHandler(flows.createDnsFlow),
      },
      {
        action: CallbackAction.DNS_LIST_DOMAIN,
        handler: new DnsListDomainHandler(flows.listDnsFlow),
      },
      ...
      {
        action: CallbackAction.DNS_DELETE_SELECT,
        handler: new DnsDeleteSelectHandler(flows.deleteDnsFlow),
      },
      {
        action: CallbackAction.DNS_DELETE_CONFIRM,
        handler: new DnsDeleteConfirmHandler(flows.deleteDnsFlow),
      },
      {
        action: CallbackAction.DNS_EDIT_SELECT_DOMAIN,
        handler: new DnsEditSelectHandler(flows.editDnsFlow),
      },
      {
        action: CallbackAction.DNS_EDIT_SELECT_RECORD,
        handler: new DnsEditRecordHandler(flows.editDnsFlow),
      },
      {
        action: CallbackAction.DNS_EDIT_FIELD,
        handler: new DnsEditFieldHandler(flows.editDnsFlow),
      },
      { action: CallbackAction.DNS_SAVE_ALL, handler: new DnsSaveAllHandler(flows.editDnsFlow) },
      {
        action: CallbackAction.DOMAIN_CREATE,
        handler: new DomainCreateHandler(flows.createDomainFlow),
      },
      ...
      {
        action: CallbackAction.NAV_MAIN_MENU,
        handler: new NavigationMainMenuHandler(flows.mainMenu),
      },
      {
